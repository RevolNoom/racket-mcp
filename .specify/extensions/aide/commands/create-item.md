...
- Always spawn a team consisting of a **worker** subagent that performs the actual work and a **reviewer** subagent that reviews the output.
- The worker repeatedly refines its work until the reviewer reports no further improvements.
- After the work item is created, the process auto‑continues to the next step without requiring user intervention.
