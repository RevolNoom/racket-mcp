...
- Always spawn a team consisting of a **worker** subagent that prepares the queue and a **reviewer** subagent that validates the generated queue.
- The worker refines the queue entries until the reviewer reports no further improvements.
- After the queue is created, the process auto‑continues to the next step without requiring user intervention.
