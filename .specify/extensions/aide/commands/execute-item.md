...
- Always spawn a team consisting of a **worker** subagent that implements the item and a **reviewer** subagent that reviews the implementation.
- The worker iteratively enhances the implementation until the reviewer finds nothing else to add.
- After the item is executed, the process auto‑continues without stopping, moving to the next step automatically.
