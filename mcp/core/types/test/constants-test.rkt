...
;; --- reserved `_meta` key strings from constants.ts (revision 2026-07-28) ---
(check-equal? (ts-const-string constants-src "PROTOCOL_VERSION_META_KEY" "constants.ts")
              PROTOCOL-VERSION-META-KEY
              "PROTOCOL_VERSION_META_KEY (constants.ts) vs PROTOCOL-VERSION-META-KEY")
(check-equal? (ts-const-string constants-src "CLIENT_INFO_META_KEY" "constants.ts")
              CLIENT-INFO-META-KEY
              "CLIENT_INFO_META_KEY (constants.ts) vs CLIENT-INFO-META-KEY")
(check-equal? (ts-const-string constants-src "CLIENT_CAPABILITIES_META_KEY" "constants.ts")
              CLIENT-CAPABILITIES-META-KEY
              "CLIENT_CAPABILITIES_META_KEY (constants.ts) vs CLIENT-CAPABILITIES-META-KEY")
(check-equal? (ts-const-string constants-src "LOG_LEVEL_META_KEY" "constants.ts")
              LOG-LEVEL-META-KEY
              "LOG_LEVEL_META_KEY (constants.ts) vs LOG-LEVEL-META-KEY")
(check-equal? (ts-const-string constants-src "RELATED_TASK_META_KEY" "constants.ts")
              RELATED-TASK-META-KEY
              "RELATED_TASK_META_KEY (constants.ts) vs RELATED-TASK-META-KEY")
