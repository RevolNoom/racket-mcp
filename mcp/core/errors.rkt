...
(require racket/contract
         ...
         (only-in "types/spec-2025-11-25.rkt"
                  jsonrpc-error
                  jsonrpc-error?
                  ;; accessors for the DECODE direction (item 007)
                  jsonrpc-error-code
...
(provide
 (contract-out
  ...
  ;; --- the DECODE function (item 007: canonical inverse of ENCODE) ---
  ;; A received jsonrpc-error struct -> the matching typed protocol exn.
  ;; ENCODE erased the exn SUBTYPE to a flat jsonrpc-error, so DECODE always
  ;; reconstructs a :protocol exn (the wire carries only the code); see the
  ;; decode comment + the round-trip test for why this asymmetry is intended.
  [jsonrpc-error->exn        (-> jsonrpc-error? exn:fail:mcp:protocol?)]))
...
;; LOCAL JSON-object predicate (byte-identical to spec-2025-11-25.rkt:51, which
;; is NOT exported — importing it would raise a compile-time "not exported"
;; error). A JSON object is an immutable symbol-keyed hasheq. Used only by the
;; two data-gate helpers below to mirror errors.ts's `data &&`-then-shape check.
(define (json-object? d) (and (hash? d) (hash-eq? d) (immutable? d)))

;; Data-gate helpers (private, NOT provided) — mirror the shape guards in
;; `ProtocolError.fromError` (errors.ts:23-35). They gate whether a special
;; code's data is "well-shaped enough to specialize"; they NEVER mutate or
;; coerce the carried data. A code-only match is NOT sufficient: with bad data
;; the decoder still produces the right CODE via the generic else branch, but
;; does not fake a specialized error whose accessors would be undefined.

;; -32042: data is a JSON object carrying an `'elicitations` key (errors.ts:24-25).
(define (url-elicitation-data? d)
  (and (json-object? d) (hash-has-key? d 'elicitations)))

;; -32004: data is a JSON object whose `'supported` is a list AND whose
;; `'requested` is a string (errors.ts:32 — Array.isArray + typeof === 'string').
(define (unsupported-version-data? d)
  (and (json-object? d)
       (list? (hash-ref d 'supported #f))
       (string? (hash-ref d 'requested #f))))

;; ---------------------------------------------------------------------------
;; DECODE: jsonrpc-error struct -> a typed exn:fail:mcp:protocol.
;;
;; The one-way inverse of ENCODE, mirroring `ProtocolError.fromError`
;; (errors.ts:21-39). A single `cond` on the received code:
;;   - -32042 with well-shaped elicitations data -> URL-elicitation-typed error
;;   - -32004 with well-shaped supported/requested data -> unsupported-version-typed error
;;   - anything else (incl. the two specials with absent/malformed data) ->
;;     a generic protocol error carrying the RECEIVED code/message/data verbatim
;;     (errors.ts:38). The received `message` is threaded through unchanged —
;;     the decode NEVER synthesizes a default message (unlike the TS subclass
;;     constructors at errors.ts:47/68, which `fromError` bypasses).
;;
;; All branches build the SAME struct (exn:fail:mcp:protocol) via
;; make-protocol-error and carry `data` by reference; the gate's only observable
;; effect today is that a malformed special code still gets the right code (kept
;; for errors.ts parity + forward-compat with a future specialized-accessor
;; layer). Codes are matched against the imported constants — never re-literaled.
;; ---------------------------------------------------------------------------
(define (jsonrpc-error->exn e)
  (define code    (jsonrpc-error-code e))
  (define message (jsonrpc-error-message e))
  (define data    (jsonrpc-error-data e))
  (cond
    ...
    [else
     (make-protocol-error code message data)]))
