...
> **Status:** ✅ Complete — delivered by `execute-item`. All acceptance boxes checked.
...
;;   NOTE: a cleanly-parsed non-eof scalar (the symbol 'null, a number, the
;;   boolean #f/#t, a string, an array) is a parse SUCCESS -> (values value #t);
;;   the ONLY failures are the three above (read-json raises, eof, or trailing
;;   non-whitespace). The skip decision keys on the ok? flag, NEVER on whether
;;   the value is truthy (JSON false parses to Racket #f — a SUCCESS, not a
;;   failure; see Part 4's #"false\n" case).
...
- **CRITICAL — a valid-JSON SCALAR line RAISES through `read-message!` (MEDIUM 7 — parse-success, not skip).** A bare scalar parses cleanly (it is NOT a `read-json` failure) but is not an object, so it fails the envelope and must RAISE, not skip: `(check-exn exn:fail? (λ () (read-buffer-read-message! (feed (make-read-buffer) #"42\n"))))`; likewise `#"true\n"`, `#"\"hi\"\n"`, **`#"null\n"`** (`read-json` returns the symbol `'null` — a parse SUCCESS, so it reaches the envelope check and raises; do NOT conflate it with the empty-line `eof` skip), and **`#"false\n"`**. **`#"false\n"` is the decisive case — assert it explicitly:** JSON `false` → `read-json` → Racket `#f`, which **collides with `try-parse-json-line`'s `(values #f #f)` failure sentinel value**. A correct decoder keys the skip decision on the **`ok?` flag** (the second value), so `false` is a parse SUCCESS (`(values #f #t)`) that reaches the envelope check and RAISES; a buggy decoder that instead tests the *value*'s truthiness would see `#f` and wrongly **SKIP** it. It is the one scalar that distinguishes a correct `ok?`-flag decoder from a value-truthiness one, so it MUST be asserted: `(check-exn exn:fail? (λ () (read-buffer-read-message! (feed (make-read-buffer) #"false\n"))))`. This pins the parse-success-vs-parse-failure boundary inside `try-parse-json-line`: only a genuine parse failure (`read-json` raises, `eof` on empty/whitespace, or trailing non-whitespace) skips; every successfully-parsed non-eof value — including `#f` — goes to the envelope check.
