...
- [x] Service started: N/A (demo script + doc closeout, no services)
- [x] Build verified: `raco make mcp/core/demo/s1-demo.rkt` → exit 0
- [x] Demo end-to-end: `racket mcp/core/demo/s1-demo.rkt` → exit 0; transcript pasted below
- [x] Demo CWD-independent: run from /tmp → exit 0
- [x] Demo tests: `raco test mcp/core/demo/s1-demo.rkt` → exit 0, 11 checks passed
- [x] Non-vacuous round-trip: un-pruned arm-1 `expect` (`init-orig`) → arm-1 check FAILED (1/11) as expected; reverted → green
- [x] Malformed arm rejects via CONTRACT (not decoder, not guard): input = `(hasheq 'name 42 'arguments (hasheq))`; decoder ACCEPTED it (returned struct), `(contract r25:call-tool-request-params/c … 'demo 'demo)` raised `string?` contract violation; error code = `-32603` (shape A, `INTERNAL-ERROR`); message matched `#rx"contract violation"`, did NOT match `#rx"unexpectedly accepted"`. Drift-verified: valid `name="ok"` → contract accepts → guard fires → 2/11 failures (the two non-fabricated assertions) → reverted green.
- [x] Full regression: `raco test mcp/core/types/ mcp/core/test/ mcp/core/demo/` → exit 0, 919 passed (908 inherited from items 001–008, 0 regressed; +11 new from the demo)
- [x] No new types in demo: `grep -cE '\(struct |define-struct|exn:fail:mcp' mcp/core/demo/s1-demo.rkt` → 0
- [x] No mcp/examples collision: `test ! -d mcp/examples` → holds
- [x] progress.md boxes: :56,:58,:59,:60,:61,:62 → [x]; :57 unchanged; no revert
- [x] Stage S1 status: header/overview-row/test-line → ✅
- [x] Parity-matrix section: updated to core/types/* + errors/* `partial`; no new roadmap table
- [x] G1 not over-claimed: G1 row not ✅
- [x] Database tables verified: N/A
- [x] API endpoints verified: N/A
- [x] Screenshots captured: N/A (no UI; transcript pasted instead — see §The transcript below)
```

### Test commands run and results

All from repo root `/home/tlam/racket-mcp`, Racket v8.18 [cs]. REAL output recorded below.
...
$ racket -e '(require (file "mcp/core/main.rkt")) (displayln (procedure? r25:json->initialize-request))'
  → #t   (barrel reaches the r25:-prefixed decoder; also verified the other 5 names + INTERNAL-ERROR=-32603)

$ raco make mcp/core/demo/s1-demo.rkt                      # the new demo
  → exit 0

$ racket mcp/core/demo/s1-demo.rkt                         # the demo transcript
  → exit 0   (transcript: parse+print+re-emit initialize & tools/call; malformed→error object code -32603; error-response envelope round-trip)
...
$ raco test mcp/core/types/ mcp/core/test/ mcp/core/demo/  # whole tree incl. demo
  → 919 tests passed, exit 0   (908 inherited, 0 regressed; +11 from the demo)
```
...
1. **Malformed-arm rejection mechanism (shape A, plain `-32603`).** Used the
   `(contract r25:call-tool-request-params/c (r25:json->call-tool-request-params (hasheq 'name 42 'arguments (hasheq))) 'demo 'demo)`
   route — NOT the bare decoder, which does not self-reject (verified live: the decoder ACCEPTS
   `name=42` and returns a struct). The decode succeeds, then the contract RAISES a `string?`
   contract violation on the `call-tool-request-params-name` field, which `exn->jsonrpc-error-jsexpr`
   maps to `code: -32603` (the non-mcp `exn:fail` branch, `errors.rkt:176`) with a message beginning
   `"…/s1-demo.rkt:94:4: contract violation\n  expected: string?\n  given: 42 …"`. Chose **shape A
   (plain `-32603` = `INTERNAL-ERROR`)**, not the optional shape-B re-wrap to `-32602`, because the
   roadmap Demo line only requires "a correct JSON-RPC error object" and the catch-all `-32603` for
   an internal contract violation is honest about provenance (a type-violation caught at the library
   boundary, not a protocol-level invalid-params the engine produced). The not-fabricated assertions
   are present in the `module+ test`: `(regexp-match? #rx"contract violation" msg)` AND
   `(check-false (regexp-match? #rx"unexpectedly accepted" msg))`. `err-obj` can ONLY be produced by
   the `(contract …/c …)` raise — the only other code path is the loud `(error 's1-demo "… Arm 3 is
   vacuous, FIX")` guard, which fails the test rather than feeding `err-obj`. **Drift-verified
   non-vacuity:** swapping the malformed input to a VALID `name="ok"` makes the contract accept,
   trips the guard, and produces 2 test failures (the two non-fabricated assertions) — confirming a
   future regression that stops the contract raising FAILS this arm, not passes it.

2. **Arm 4 (error-response envelope round-trip) — INCLUDED.** It complements arm 3's *constructed*
   error object with a *full JSON-RPC error envelope* (`error-response.json`, code `-32602`)
   round-tripping struct↔JSON, keeping the transcript coherent (shows both error directions). It
   adds one `jsexpr=?` check; no dropped-key handling needed (verified — the envelope round-trips
   against its raw fixture).

3. **Final `raco test` command + REAL counts.** Baseline `raco test mcp/core/types/ mcp/core/test/`
   → **908 passed** (re-run, matches item 008). Demo alone `raco test mcp/core/demo/s1-demo.rkt` →
   **11 passed**. Whole tree `raco test mcp/core/types/ mcp/core/test/ mcp/core/demo/` →
   **919 passed**, exit 0 (908 inherited, 0 regressed, +11 new). The green baseline command is
   EXTENDED to include `mcp/core/demo/` — that is the canonical S1 command going forward.

4. **Demo location.** `mcp/core/demo/s1-demo.rkt` exactly as specified — adjacent to the S1 code it
   exercises, NOT under `mcp/examples/` (S9/M15 territory, still absent). New `mcp/core/demo/`
   directory created; leaves room for later `s2-demo.rkt` etc.

5. **Barrel name resolution.** All four load-bearing names resolved through the item-008 barrel
   `mcp/core/main.rkt` under the expected `r25:` prefix: `r25:json->initialize-request`,
   `r25:json->call-tool-request`, `r25:json->call-tool-request-params` (all `procedure?`→#t), and
   `r25:call-tool-request-params/c` (`contract?`→#t). Plus `r25:json->jsonrpc-error-response` /
   `r25:jsonrpc-error-response->json`, the serializers, `exn->jsonrpc-error-jsexpr`,
   `make-protocol-error`, and `INTERNAL-ERROR` (=-32603). No façade-vs-per-revision rerouting was
   needed. `make-protocol-error` is imported but unused (shape A chosen); kept in the `only-in` list
   as documentation of the available shape-B path — harmless, and removing it would obscure the
   recorded decision.

6. **Transcript structure — `module+ main`, not top-level prints.** Put the printed transcript in a
   `(module+ main …)` so `racket mcp/core/demo/s1-demo.rkt` shows it while `raco test` (which runs
   the `test` submodule, not `main`) stays free of transcript noise, and a `require` of the demo
   would pull in neither. Cleaner separation than top-level `printf`s; the bindings the transcript
   and the tests share (`init-rt`, `err-obj`, …) are module-level `define`s computed once.

7. **`progress.md` "Parity matrix progression" replacement text.** Replaced the "Current state:
   **no rows yet (no source).** 📋" line with text marking `core/types/*` and `errors/*` as
   **`partial`** (structs/contracts/façade/guards/constants/error-layer exist and round-trip TS-SDK
   fixtures, witnessed by the demo; full cross-SDK conformance deferred to S9 §9.1/§9.2), all other
   rows `📋`, section icon advanced 📋→🚧. Justification for `partial` not `done`: the roadmap
   criterion (`roadmap.md:97`) literally says `partial` ("structs exist; exercised by conformance
   later").

8. **No new `roadmap.md` table; G1 NOT over-claimed.** Confirmed NO materialized parity table was
   added to `roadmap.md` (none exists; the prose criteria + progress.md section ARE the matrix at
   S1). The "Vision goal coverage" G1 row (`progress.md:298`) and the NFR rows were left UNTOUCHED —
   S1 only partially satisfies G1 (full wire-parity is S9-certified); marking G1 ✅ here would
   over-claim. Those tables advance at S9.
