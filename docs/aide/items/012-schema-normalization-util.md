...
- [x] `mcp/core/util/schema.rkt` exists as `#lang racket/base` (or `#lang racket`) with an explicit, curated `provide` (no `(provide (all-defined-out))`). The `mcp/core/util/` collection directory is created by this item.
- [x] The module exports `normalize-schema` (the entry point), the `normalized-schema` struct **predicate** + curated **accessors** (`normalized-schema-wire`, `normalized-schema-handle`, `normalized-schema-validate`), the `normalized-schema-prompt-arguments` helper, and the Form-B **object-descriptor surface** `object-schema/c` (+ its predicate). It does NOT re-export the M3 result API (callers `require` `provider.rkt` for `validation-ok?` etc.) and does NOT leak internal mapping helpers.
- [x] **Form A — JSON Schema input → wire + handle.** `(normalize-schema (hasheq 'type "object" 'properties (hasheq 'name (hasheq 'type "string")) 'required '("name")))` returns a `normalized-schema?` whose `normalized-schema-wire` has root `type:"object"` and whose `normalized-schema-handle` is a `compiled-validator?`. `(normalized-schema-validate ns (hasheq 'name "John"))` → `validation-ok?`; `(normalized-schema-validate ns (hasheq))` → `validation-errors?`.
- [x] **Form A — root `type:"object"` invariant (all branches pinned).** A Form-A schema with NO root `type` gets `type:"object"` added (TS `{type:"object", ...result}` parity). A Form-A schema with root `type:"object"` is unchanged (modulo the ensured key). A Form-A schema with an explicit **non-object** root `type` is **rejected** at `normalize-schema` (`check-exn`, via an S1 error) — tested with **`(hasheq 'type "string")` AND a representative non-string non-object `(hasheq 'type "array")`** (case 1c), so the reject path is not string-specific.
- [x] **Form B — root rule is form-dependent (pinned).** A bare scalar contract root (`string?`) → **reject**; a root **array** contract `(listof string?)` → **reject** (case 1a, array root non-object); a root **enum** contract `(or/c "a" "b")` → **reject** (case 1b — a typeless contract-derived root is NOT auto-wrapped, unlike a typeless Form-A input which gets `type:"object"` added). The form-dependence (Form-A typeless → wrap; Form-B typeless → reject) is documented in Decisions.
- [x] **Form B — flat-contract object descriptor → wire + handle.** Normalizing the object descriptor `{name: string? (required), age: exact-integer? (optional)}` (in the chosen surface) yields a wire JSON Schema `equal?` (or structurally equal, per the pinned comparison) to `(hasheq 'type "object" 'properties (hasheq 'name (hasheq 'type "string") 'age (hasheq 'type "integer")) 'required '("name"))`, and a handle that accepts `{name:"a", age:5}` and `{name:"a"}` and rejects `{age:5}` and `{name:5}`.
- [x] **Form B — supported flat-contract scalar mappings.** Each of `string?`→`{type:"string"}`, `exact-integer?`→`{type:"integer"}`, `real?`/`number?`→`{type:"number"}`, `boolean?`→`{type:"boolean"}` maps as tabled (verified by inspecting the wire fragment a one-field object descriptor produces). `(listof string?)`→`{type:"array", items:{type:"string"}}`. A string/number/bool/null-literal `or/c` (e.g. `(or/c "red" "green")`) → `{enum:["red","green"]}`.
- [x] **Dual-form equivalence (the headline criterion, queue-mandated).** A Form-B contract input and an *equivalent* Form-A JSON-Schema input (per "Equivalent" above) produce handles that **accept the same set of values and reject the same set of values**: for the agreed accept-set, BOTH handles return `validation-ok?`; for the agreed reject-set, BOTH return `validation-errors?`. **AND** the two emitted **wire JSON Schemas match** (`equal?` / structural equality, after root-`type:"object"` normalization). Assert both halves (same verdicts + same wire) explicitly. This is the queue's key testable.
...
**(k) Post-build outcomes (recorded at implementation).**
- **Wire-schema comparison:** exact `equal?` survived — `(equal? (wire ns-contract) (wire ns-json))` holds because `hasheq` equality is content/order-independent and `required` is the same `'("name")` list both sides. No field-by-field fallback was needed.
- **Contract recognition mechanism (deviation from the spec's suggested surface, forced by the tests).** The Testing Strategy exercises REAL `racket/contract` combinators (`(or/c "red" "green")`, `(listof string?)`) as field contracts, not a bespoke descriptor-surface `or/c`/`listof`. A compiled `or/c`/`listof` exposes no public arm-decomposition API, so recognition reads each flat contract's **`contract-name` DATUM** (a structured S-expression, NOT a string parse): scalar predicates are matched by their name symbol (`string?`/`exact-integer?`/`real?`/`rational?`/`number?`/`boolean?`); `(listof X)`/`(or/c …)` by the list head; literal vs predicate `or/c` arms by self-quoting datum (string/number/bool) or a `(quote sym)` wrapper (how `contract-name` renders a literal symbol like the `(json-null)` member `'null`, distinct from an unquoted predicate reference). A nested `object-schema/c` is a `prop:flat-contract` struct whose **`contract-name` is the descriptor struct itself**, so `(listof <descriptor>)` recovers the descriptor for recursion (verified: `(listof inner)` → `items` of an object sub-schema with path `'("items" 0 "id")`). Descriptors are detected by `object-schema/c?` BEFORE the contract path (a descriptor is also a flat contract).
- **Require list as built (S1 + M3 only):** `racket/contract` (introspect flat contracts via `contract-name` + the `prop:flat-contract`/`build-flat-contract-property` for `object-schema/c`), `racket/list` (`remove-duplicates` for enum de-dup), `"../main.rkt"` (S1 barrel → `make-protocol-error`), `"../validators/provider.rkt"` (M3 port → `provider-compile`/`validate`/`compiled-validator?`), `"../validators/from-json-schema.rkt"` (M3 default → `make-racket-native-provider`). NO `json` needed (the module carries `(json-null)` through as the symbol `'null` without referencing it). NO `net/url`.
- **Restricted-load result:** the walk rooted at `schema.rkt` has EMPTY intersection with the banned set (`racket/system racket/port racket/tcp racket/udp net/url net/http-client net/sendurl racket/sandbox`). **Drift / non-vacuity:** a scratch copy with `(require racket/tcp)` injected made the walk report `racket/tcp` hit `#t`, then reverted — the test is non-vacuous.
- **Check count:** `raco test mcp/core/util/` → **102 checks pass, 0 failures, 0 errors**. M3 untouched: `raco test mcp/core/validators/` → **300 pass**. `raco make mcp/core/util/schema.rkt` → exit 0, clean. Smoke check `(#t #t)` as specified.
...
- [x] **Build/compile succeeds:** `raco make mcp/core/util/schema.rkt` compiles with no errors/warnings.
- [x] **Module loads in isolation:** `racket -e '(require (file "mcp/core/util/schema.rkt"))'` from repo root succeeds.
- [x] **Tests pass:** `raco test mcp/core/util/test/schema-test.rkt` → all checks pass, exit 0.
- [x] **Collection tests pass:** `raco test mcp/core/util/` → exit 0.
- [x] **M3 untouched:** `raco test mcp/core/validators/` → still exit 0 (this item does not modify M3).
- [x] **Services started:** N/A (no services — pure library).
- [x] **Application runs:** N/A (library; "running" = the require + REPL/normalize smoke check below).
- [x] **Feature verified (REPL / normalize smoke check):** from repo root, normalize a JSON Schema and validate a good + bad value — e.g.
...
- [x] **Form A verified:** JSON-Schema input → wire (root `type:"object"`) + handle; ok/bad values validate.
- [x] **Form A root invariant verified (both reps):** no-root-`type` → `type:"object"` added; object root → unchanged; non-object root rejected for BOTH `{type:"string"}` AND `{type:"array"}` (case 1c).
- [x] **Form B root rule verified (form-dependent):** bare scalar root → reject; root `(listof string?)` → reject (case 1a); root `(or/c "a" "b")` → reject (case 1b, NOT auto-wrapped).
- [x] **Form B object descriptor verified:** `(object-schema/c (hash 'name string? 'age exact-integer?) #:required '(name))` → wire equals the equivalent JSON Schema; `properties` keys are symbols, `required` members are strings.
- [x] **Form B scalar/array/enum mappings verified:** string/integer/number/boolean; `(listof string?)`→array+items; all-literal `(or/c "red" "green")`→enum; single-arm + duplicate de-dup; **mixed `(or/c "a" string?)` rejected (case 5)**; all-predicate `(or/c string? number?)` rejected.
- [x] **`exact-integer?` self-consistency verified (case 6):** an `exact-integer?` field handle rejects `5.0`, accepts `5`; `integer?` as a field contract is rejected.
- [x] **Dual-form equivalence verified (headline):** contract input and equivalent JSON-Schema input accept the same value set + reject the same value set, AND emit the same wire schema.
- [x] **Delegation parity verified (case 2 + 2b):** handle agrees with `(provider-compile P (wire-of X))` on a **typeless Form-A** input (normalize-THEN-compile) AND on the **contract form** (self-delegation over the full sample set).
- [x] **Deferred keyword pass-through verified (case 3):** Form-A `{...minLength:3...}` → wire retains `minLength`; handle accepts a short string; `provider-warnings-for` records `'minLength`.
- [x] **No-equivalent contract rejected:** `(-> string? string?)` / `(->i …)` / `struct/c` / bare `even?` / `integer?` as a field contract → `normalize-schema` raises an S1 error (not silent `{}`, not a raw contract crash).
- [x] **`and/c` rejected: raises (case 7):** `(and/c string? immutable?)` field → raises.
- [x] **Optional vs required verified:** required field in wire `required` (string); optional absent OK; optional-present-wrong-type rejects.
- [x] **`object-schema/c` absent-required rejected (case 4):** `#:required '(missing)` naming an undeclared field → raises at construction.
- [x] **Nested object descriptors verified:** nested `object-schema/c` field → nested `{type:"object",…}`, located path on inner failure; `(listof <descriptor>)` → object `items`; higher-order Racket contract always rejected.
- [x] **Empty schema → `validation-ok?` verified (case 9):** Form-A `(hasheq)` AND Form-B empty descriptor each yield `validation-ok?` (asserted explicitly, not just no-exn) on `(hasheq)` and `(hasheq 'x 1)`.
- [x] **String-keyed hash rejected (case 8):** `(hash "type" "object")` (string-keyed `equal?`-hash) → `normalize-schema` raises (out of contract, not silently mis-validated); `42`/`"x"` also raise.
- [x] **Provider injection + default verified:** default `make-racket-native-provider`; custom/stub provider routed; deferred-keyword `provider-warnings-for` proves compile-through-provider.
- [x] **`normalized-schema-validate` sugar verified:** equals `(validate (normalized-schema-handle ns) v)`.
- [x] **Prompt-arguments helper verified (ships here):** entries per property with required flags; mirrors TS.
- [x] **No `(module+ test …)` in `schema.rkt` confirmed:** tests live in `test/schema-test.rkt`.
- [x] **Portability verified:** the restricted-load test passes (no subprocess/socket — incl. NO `net/url` — in the transitive import closure of `schema.rkt`).
...
- [x] Application started successfully: N/A (library; `require` + normalize smoke check → `(#t #t)`)
- [x] Build verified: `raco make mcp/core/util/schema.rkt` clean (exit 0, no warnings)
- [x] Module load verified: `(require (file ".../schema.rkt"))` succeeds (smoke check ran)
- [x] Tests verified: `raco test mcp/core/util/` → 102 checks pass, 0 failures, 0 errors
- [x] M3 untouched: `raco test mcp/core/validators/` → 300 pass, still exit 0
- [x] Form A verified: JSON-Schema input → wire (root type:object) + handle; ok/bad validate
- [x] Form A root invariant (both reps): no-type→added; object→unchanged; non-object→rejected for {type:string} AND {type:array} (case 1c)
- [x] Form B root rule (form-dependent): bare scalar→reject; (listof string?)→reject (1a); (or/c "a" "b")→reject (1b)
- [x] Form B object descriptor verified: wire equals equivalent JSON Schema; properties symbol-keyed, required string members
- [x] Form B scalar/array/enum mappings verified; mixed/all-predicate or/c rejected (case 5)
- [x] exact-integer? self-consistency: int field rejects 5.0, accepts 5; integer? rejected (case 6)
- [x] Dual-form equivalence verified (headline): same accepts + same rejects + same wire (exact equal?)
- [x] Delegation parity verified: handle == (provider-compile P (wire-of X)) on typeless Form-A (case 2) + contract self-delegation over full sample set (case 2b)
- [x] Deferred keyword pass-through (case 3): wire retains minLength; handle accepts short string; provider-warnings-for records 'minLength
- [x] No-equivalent contract rejected: higher-order (->)/dependent (->i)/struct/c/opaque (even?)/integer? → exn:fail:mcp error (not silent {})
- [x] and/c rejected: raises (case 7): (and/c string? immutable?) AND (and/c string? (string-len/c 10)) fields → raise
- [x] Optional vs required verified (missing required reject; optional absent ok; optional present-wrong-type reject)
- [x] object-schema/c absent-required rejected at construction (case 4)
- [x] Nested object descriptors verified: nested {type:object,…} + located path '("outer" "inner"); (listof descriptor)→items, path '("items" 0 "id"); higher-order in listof always rejected
- [x] Empty schema → validation-ok? (case 9): Form-A {} + Form-B empty descriptor each validation-ok? on {} and {x:1} (asserted, not just no-exn)
- [x] String-keyed hash rejected (case 8): (hash "type" "object") → raises; mutable hasheq, 42, "x" → raise
- [x] Provider injection + default verified (default provider; custom accept-all stub honoured; provider-warnings-for proves compile-through-provider)
- [x] normalized-schema-validate sugar verified (equal? to (validate (handle ns) v))
- [x] prompt-arguments helper verified (ships here): name required, age optional, description surfaced
- [x] No (module+ test …) in schema.rkt confirmed (tests in test/schema-test.rkt)
- [x] Portability verified: restricted-load walk over schema.rkt — empty intersection with banned set (incl. no net/url)
- [x] Portability drift check: injected (require racket/tcp) → walk reported racket/tcp hit #t, then reverted
- [x] S1 + M3 imports confirmed: require list = racket/contract + racket/list + ../main.rkt + ../validators/provider.rkt + ../validators/from-json-schema.rkt (json NOT needed)
