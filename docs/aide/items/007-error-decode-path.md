...
- [x] Decode -32042 verified: → protocol-error? #t, code = URL-ELICITATION-REQUIRED, data preserved
- [x] Decode -32004 verified: → protocol-error? #t, code = UNSUPPORTED-PROTOCOL-VERSION, data preserved
- [x] Decode unknown-code verified: -39999 (and INVALID-PARAMS) → generic protocol-error?
      carrying the RECEIVED code (not defaulted), auth-error? #f
- [x] Data-gate NON-VACUOUSLY verified: -32042 with object-but-no-'elicitations (C2) and -32004
      with valid 'supported but broken/absent 'requested (C3) → generic branch, right code, no
      throw — a code-only impl would fail these
- [x] Message-no-synth verified: -32042 with "" message + good data → exn-message "" (not synthesized)
- [x] Round-trip verified: (jsonrpc-error->exn (exn->jsonrpc-error e)) preserves code/message/data
...
- [x] Raisable/catchable verified: decoded exn catchable by protocol-error? handler; exn:fail?/mcp-error? #t
- [x] Data-carriage verified: falsy/odd/nested data carried verbatim; absent stays absent
- [x] Contract verified: raw hasheq input → exn:fail:contract? (canonical input is the struct)
- [x] Codes-imported verified: no bare -32042/-32004 literals in decode code (constants only)
- [x] Additive-only verified: changes confined to DECODE anchor + second provide block + the
      `only-in` require line for the three jsonrpc-error accessors (errors.rkt is new/untracked in
      this branch, so `git diff` is empty; reviewed by inspection — item 006's first provide block
      and the encode logic are untouched)
- [x] Portability verified: require list = racket/contract + types/constants.rkt + types/spec-2025-11-25.rkt
- [x] Drift detection: flipped a decode assertion (du code → UNSUPPORTED-PROTOCOL-VERSION) → FAILURE printed; reverted → clean
- [x] Regression: constants-test / guards-test / spec-2025-11-25-test → exit 0, 0 FAILURE/error lines
- [x] Database tables verified: N/A
- [x] API endpoints verified: N/A
- [x] Screenshots captured: N/A (no UI)
```

### Test commands run and results (delivered 2026-06-19)

- `racket mcp/core/test/errors-test.rkt` → exit 0, no FAILURE, prints
  "errors-test.rkt: all checks executed" (item-006 encode Parts 1–5 + new decode Part 6).
- `racket -e '(require (file "mcp/core/errors.rkt"))'` → loads clean.
- REPL smoke: decode -32042+good-data → code -32042, protocol-error? #t; decode -39999 → code
  -39999; round-trip `(jsonrpc-error->exn (exn->jsonrpc-error (make-protocol-error -32602 "bad")))`
  → -32602; raw hasheq input → REJECTED (exn:fail:contract?).
- `grep -nE '\-32042|\-32004' mcp/core/errors.rkt` → all matches are in comments only; the decode
  `cond` uses URL-ELICITATION-REQUIRED / UNSUPPORTED-PROTOCOL-VERSION.
- Drift: flipped the `du` code assertion → `FAILURE` printed; reverted → clean.
- Regressions: `racket mcp/core/types/test/constants-test.rkt` (exit 0),
  `.../guards-test.rkt` (exit 0), `.../spec-2025-11-25-test.rkt`
  (exit 0, "pinned check count = 120").
...
- **Optional wire wrapper OMITTED (YAGNI, as recommended).** Only `jsonrpc-error->exn :
  (-> jsonrpc-error? exn:fail:mcp:protocol?)` was added. No `jsonrpc-error-jsexpr->exn`, so no
  `only-in json->jsonrpc-error` import — S3 calls `json->jsonrpc-error` itself. The
  contract-rejection test pins that the canonical input is the struct.
- **`json-object?` = LOCAL helper (option (b), as required).** Defined byte-identically in
  errors.rkt; no M1 edit, no new import for the gate. The two private gate predicates
  (`url-elicitation-data?`, `unsupported-version-data?`) build on it; neither is provided.
- **One require-line change beyond the gate decision:** the `only-in` from
  `spec-2025-11-25.rkt` gained `jsonrpc-error-code` / `jsonrpc-error-message` /
  `jsonrpc-error-data` (the struct accessors the decoder reads — item 006 imported only the
  constructor + predicate + serializer). This is the SAME module (no new module dependency), so
  the Portability NFR holds; the require list is still racket/contract + types/constants.rkt +
  types/spec-2025-11-25.rkt.
- **All AC met; no deviations from the spec's build contract.** The three-way `cond` is exactly
  as specified; every branch builds `make-protocol-error`; codes are the named constants.
