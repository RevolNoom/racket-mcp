...
> **Status:** ✅ Delivered (execute-item 2026-06-18).
...
### Decisions settled during execute-item (2026-06-18)

- **shared-helpers:** imported `(only-in "spec-2025-11-25.rkt" absent absent? present?)` and
  **re-exported** them — 003 and 004 share ONE `absent` sentinel binding (005 unions both against
  the same `eq?` sentinel). The tiny internal helpers (`json-object?`, `put`/`put!`, `split-loose`,
  `hash-merge`, `opt/c`, `lit/c`, etc.) were **duplicated inline** (003 does not provide them);
  003 was **NOT** modified.
- **reserved-key constants:** added the 5 `*-META-KEY` defines to `constants.rkt` **additively**
  (exact strings from `constants.ts` 5/14/19/27/38; item 001's row stays ✅). `constants-test.rkt`
  was extended with 5 byte-for-byte parity `check-equal?`s against the TS source (22 → 27 checks);
  it does **not** pin a provided-binding count, so the additions are non-breaking. The error codes
  `-32004`/`-32003` were reused from constants.rkt (NOT re-added).
- **`_meta` required-ness / asymmetry:** followed the `.d.ts` — `read-request-meta` **rejects** a
  request params with no `_meta`; `request-meta/c` requires the 3 required reserved keys. `result-type`
  modeled OPTIONAL (`(opt/c result-type/c)`, absent treated as "complete").
- **module load order:** content blocks + the `resource-contents` family are defined EARLY (before
  `resource`) because `embedded-resource` references `resource-contents/c`. The `input-request/c` /
  `input-response/c` unions are **lazy** (`flat-named-contract` wrapping a `lambda`) so they can name
  the later-defined input request/result structs; `input-requests/c` / `input-responses/c` are defined
  early (after §D) so the read-resource/get-prompt/tools-call params contracts can reference them.
- **typed errors:** the 5 code-pinned errors are predicates over the single `jsonrpc-error` struct
  (`parse-error?` … `internal-error?`); the 2 data-carrying errors get constructor + predicate
  (predicate also shape-checks `data`), mirroring 003's URL-elicit pattern.
- **ListRootsResult BARE:** modeled as `(struct list-roots-result (roots))` — serializer emits
  EXACTLY `{"roots":[...]}`, no `meta`/`result-type`/`rest`; per-root `_meta` round-trips on `root`.
- **Counts:** 94 transparent structs, 120 provided `…/c` contracts, 104 `json->` deserializers
  (+ matching `->json` serializers), 8 aggregate/union contracts (incl. `input-request/c`,
  `input-response/c`; NO `server-request/c`). Test: **176** checks pass; full
  `raco test mcp/core/types/` = **640**; `constants-test` stays green (27).
