...
```markdown
## Validation Results (completed 2026-06-19)
- [x] Service started: N/A (pure-data module, no services)
- [x] Application started successfully: N/A (library; `require` succeeds)
- [x] Build verified: `racket -e '(require (file "mcp/core/errors.rkt")) ...'` succeeds
      (`raco make` skipped — raco broken in sandbox; documented in Testing Prerequisites)
- [x] Module load verified: `(require (file ".../errors.rkt"))` succeeds in isolation
- [x] Tests verified: `racket mcp/core/test/errors-test.rkt` → exit 0, 0 FAILURE lines,
      prints "errors-test.rkt: all checks executed" (≈60 rackunit checks, well above the
      ~25 minimum)
- [x] Hierarchy verified: 3 exn subtypes (exn:fail:mcp / :protocol / :auth) subtype exn:fail
- [x] Construction-with-code verified: each subtype constructs with its stable constants.rkt
      code (=-asserted against the imported binding, never a literal)
- [x] Predicate-matrix verified: mcp?/protocol?/auth? discriminate; generic exn (synthetic +
      caught `(car '())`) ⇒ all #f; raised mcp error catchable by `mcp-error?` handler
- [x] Encode verified: mcp subtype → own code/message/data; non-mcp exn (synthetic
      `make-exn:fail` AND real caught `vector-ref`) → INTERNAL-ERROR (-32603) fallback
- [x] Wire-shape verified: serialized hasheq has 'code/'message; absent data OMITTED (not
      'null); falsy data (#f/'null/0/""/empty-hash/nested) survives verbatim; composes into
      an envelope that satisfies guards' `is-jsonrpc-error?`
- [x] Codes-imported verified: `grep -nE '\-326..|\-3200.|\-32042|\-32700' mcp/core/errors.rkt
      | grep -v ';;'` is EMPTY — all numeric codes appear only in comments
- [x] Portability verified: require list is exactly racket/contract + types/constants.rkt +
      types/spec-2025-11-25.rkt — no subprocess/socket; spec + types.rkt façade still load
- [x] 007-readiness verified: "DECODE (item 007)" anchor present; second `provide` block is
      append-only; `make-protocol-error URL-ELICITATION-REQUIRED … (hasheq 'elicitations '())`
      constructs a typed -32042 error with no new struct
- [x] Drift detection: flipped fallback assertion (INTERNAL-ERROR → INVALID-PARAMS) → FAILURE
      printed; reverted → clean
- [x] Regression: `racket mcp/core/types/test/constants-test.rkt` and
      `.../guards-test.rkt` → 0 FAILURE/error lines
- [x] Database tables verified: N/A
- [x] API endpoints verified: N/A
- [x] Screenshots captured: N/A (no UI)
```

### Test commands run and results (2026-06-19)

- `racket mcp/core/test/errors-test.rkt` → exit 0, no FAILURE lines, prints
  "errors-test.rkt: all checks executed".
- `racket mcp/core/types/test/constants-test.rkt` → exit 0, no FAILURE/error lines (regression).
- `racket mcp/core/types/test/guards-test.rkt` → exit 0, no FAILURE/error lines (regression).
- Drift: flipped the -32603 fallback assertion → `FAILURE` printed; reverted → clean.
- `racket -e '(require (file "mcp/core/errors.rkt")) ...'` → loads; smoke checks return
  `-32602`, `#f` (protocol-error? on a base error), `-32603` (fallback).
- `grep -nE '\-326[0-9][0-9]|\-3200[0-9]|\-32042|\-32700' mcp/core/errors.rkt | grep -v ';;'`
  → empty (no re-literaled codes in code).

### Deviation note

One file outside `errors.rkt`/`errors-test.rkt` was touched: `types/spec-2025-11-25.rkt`'s
`provide` gained `jsexpr-value?` (additive — the predicate was already defined there, just
not exported). This lets `errors.rkt` reuse the exact value predicate behind `jsonrpc-error/c`
instead of re-defining it. Verified the spec module and the `types.rkt` façade still load.

---

## Decisions & Trade-offs

### Implementer decisions on delivery (2026-06-19)

- **Where the `jsonrpc-error` struct comes from — chose option (a) (import from item
  003).** `errors.rkt` does `(require (only-in "types/spec-2025-11-25.rkt" jsonrpc-error
  jsonrpc-error? jsonrpc-error->json jsexpr-value? absent absent?))`. ONE `jsonrpc-error`
  type and ONE serializer across the codebase; item 007 consumes the same struct
  symmetrically. The require list is exactly `racket/contract` + `types/constants.rkt`
  (codes only) + `types/spec-2025-11-25.rkt` — NO subprocess/socket module, NO I/O at load,
  so the Portability NFR holds (verified: spec + types façade still load after the edit).
- **`jsexpr-value?` added to spec-2025-11-25.rkt's `provide`.** The constructor's `data`
  contract is `(or/c absent? jsexpr-value?)`, reusing the EXACT predicate the spec's
  `jsonrpc-error/c` uses (`spec-2025-11-25.rkt:67`). `jsexpr-value?` was already defined
  there but not exported; the change is purely additive (one provide line + a comment). No
  re-definition of the value predicate in `errors.rkt`.
- **`contract-out` on the three `make-*` constructors + on the two encode functions.**
  Bad code (non-integer) / bad message (non-string) are caught at construction
  (`exn:fail:contract?`), tested by advisory-1 `check-exn` cases. The structs themselves are
  provided RAW via `(struct-out …)` (following item 003's no-`contract-out`-on-structs
  choice). `exn->jsonrpc-error` is `(-> exn? jsonrpc-error?)`; `exn->jsonrpc-error-jsexpr`
  is `(-> exn? hash?)`.
- **`#:marks` kept on the public surface.** All three constructors take an optional
  `#:marks` defaulting to `(current-continuation-marks)`; a test asserts the field is a
  populated `continuation-mark-set?` (advisory 5). 007 does not need it but it is harmless
  and keeps constructed errors stack-bearing for `error-display-handler`.
- **Advisory decision notes (totality / breaks / fallback input):** `exn-message` is total
  over `exn?`, so the `else`/fallback branch can never emit a non-string message — no guard
  needed there. `exn:break` would map to `-32603` ONLY if it reaches the encoder; S3 is
  expected to let breaks PROPAGATE, so the encoder deliberately does not special-case or
  swallow them (documented in the encoder comment). The fallback tests always pass an `exn`
  (synthetic `make-exn:fail` and a real caught `vector-ref`); a non-`exn?` raised value is a
  caller/boundary responsibility (the `(-> exn? …)` contract), not a `-32603` case.
- **Falsy `data` carriage confirmed (advisory 2):** `#f`, `'null`, `0`, `""`, empty
  `hasheq`, and a nested object/array all survive into the wire object verbatim; the nested
  case is asserted `check-eq?` (copied by reference, not inspected/flattened). Only `absent`
  is omitted. The encode branch uses `(mcp-error? e)`, not a truthiness test, so falsy data
  is never dropped.
