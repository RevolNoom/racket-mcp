...
```markdown
## Validation Results (filled during execute-item)

### Counts (Testing Part 6)
- Façade structs: **58** (`grep -c '^(struct facade-' types.rkt`).
- `normalize-facade-*` functions: **85**.
- `denormalize-facade-*` functions: **93**.
- `types.rkt`: 1476 lines. `types-test.rkt`: 348 lines (pinned check count 95;
  the test prints `façade struct predicates counted = 58`).

### Exact test commands + results (run from repo root)
- `racket mcp/core/types/test/types-test.rkt`
  → exit 0; stdout only the two inventory printfs
  (`façade struct predicates counted = 58`, `pinned check count = 95.`); no
  FAILURE/ERROR lines. **PASS.**
- `racket mcp/core/types/test/spec-2025-11-25-test.rkt` → exit 0
  (`pinned check count = 120.`). **PASS (no regression).**
- `racket mcp/core/types/test/spec-2026-07-28-test.rkt` → exit 0
  (`pinned check count = 176.`). **PASS (no regression).**
- `racket -e '(require (file ".../mcp/core/types/types.rkt"))'` → exit 0
  (module loads, no I/O at load).

### Checklist
- [x] Service started: N/A (pure-data normalization module)
- [x] Application started: N/A (library; `require` succeeds, exit 0)
- [x] Build verified: `racket -e '(require (file ".../types.rkt"))'` succeeds (raco wrapper broken)
- [x] Module load verified: require returns without error
- [x] Tests verified: `racket mcp/core/types/test/types-test.rkt` → exit 0, 0 fail/err
- [x] Collection tests verified: each test file run directly → all exit 0 (003/004/005)
- [x] Inventory verified: every primitive in §4 Groups 0–8 has a façade struct/`…/c` +
      normalize/denormalize seam. Façade struct count recorded: **58**.
- [x] SAME-FAÇADE verified (the queue's core claim): 2025 + 2026 `tools/call`, `tools/list`
      result, content block all normalize to the SAME façade struct type
- [x] Part-0 fixtures authored: list-roots-result (2025 +_meta / 2026 bare), task-bearing 2025
      tools/call (`tools-call-request-task.json`), a 2025 tool with execution (`tool-with-exec.json`),
      elicit-form-params (2025 task+_meta / 2026 bare), an inputResponses-bearing 2026 tools/call
      (existing `2026-tools-call-request.json`), create-message params (2025/2026), 2026-tool-no-exec
- [x] PRESENT/ABSENT matrix verified: RC-only fields absent on 2025 façade, present on 2026;
      2025-only fields (task / tool execution / list-roots result-meta / elicit-params task+meta)
      absent on 2026 façade, present on 2025
- [x] meta-field TYPE verified (C1/C2): 2026 call-tool meta is facade-request-meta?; 2026
      create-message meta is plain `json-object?` NOT facade-request-meta?; 2026 elicit-form-params
      has task AND meta absent (and its meta is NOT the envelope)
- [x] Lossless-on-home verified: normalize→denormalize-to-R→->json is jsexpr=? to the fixture
      for 2025 + 2026 × request (call-tool params) + result (list-tools)
- [x] `rest` parity verified (C4): non-empty rest survives home-revision round-trip (both revs,
      via unknown-key + _meta presence); rest passes through on cross-revision denormalize without
      raising (named 2026-only fields cleared via struct-copy); empty rest is {} not absent, no
      phantom _meta key introduced
- [x] Cross-revision refusal verified: denormalize-to-2026 of a 2025-only-field-bearing façade
      raises (task / tool execution / elicit task+meta / list-roots meta); denormalize-to-2025 of a
      2026-only-field-bearing façade raises (ttl-ms/cache-scope/result-type); 2025-only standalone
      (initialize) refuses to-2026; 2026-only standalone (discover) refuses to-2025; all-absent
      denorms to both without raising; bare 2026 shapes emit exactly {mode,message,requestedSchema}
      and {roots}
- [x] Aggregate unions verified: facade-client-request/c accepts initialize(2025) + discover(2026)
      + tools/call (both revs) via `contract-first-order-passes?`
- [x] Revision-parameterized dispatch verified (S4): `dispatch-for "tools/call" 2025` vs `2026`
      yields DIFFERENT parser AND normalizer pairs (`(eq? …)` is #f), each producing the correct
      revision-shaped façade (2025→task present, 2026→input-responses present); single-revision
      methods resolve only for their home revision (`initialize`/`server/discover` ↔ #f for the
      other); roots/list resolves both
- [x] Group-0 aliasing-trap verified (S5): a 2026-built text-content satisfies the SAME
      facade-text-content? predicate as the 2025-built one
- [x] Presence-vs-revision-capability verified: a 2026 result omitting result-type normalizes to
      absent (same sentinel as 2025) without crash and denormalizes back to 2026; revision identity
      carried by the explicit version tag, not the field value
- [x] `absent` identity verified: façade `absent` eq? to 003's AND 004's `absent`; optional-absent
      (omitted `tool.title` on a 2026 message) is the SAME sentinel as revision-absent
- [x] N1-readiness / boundary verified: façade owns fresh public structs (Group-0 convert-both,
      not alias); does not leak 003/004 internal struct types as the public surface; `absent`/
      `absent?`/`present?` + shared scalar contracts re-exported
- [x] Drift detection: flipped the `task absent 2026` assertion to `present?` → test prints FAILURE;
      reverted → exit 0, all pass
...
### Decisions made during implementation (iteration-002 → delivered)

These resolve the seeded recommendations below to what was actually built in
`mcp/core/types/types.rkt` + `mcp/core/types/test/types-test.rkt`:

1. **Prefix imports `r25:`/`r26:` + `absent` from 003.** `absent`/`absent?`/`present?` are imported
   via `(only-in "spec-2025-11-25.rkt" absent absent? present?)` and re-exported; `json-object?`
   is RE-DEFINED locally (003/004 do not provide it) so the façade contracts can name it. Verified
   `(eq? absent r25:absent)` and `(eq? absent r26:absent)` in the test.
2. **Group-0 = fresh `facade-` structs, both revisions CONVERTED (S5).** Every façade struct is a
   fresh `#:transparent` struct; no 003/004 struct is aliased. The aliasing-trap guard test
   confirms a 2026-built `facade-text-content` satisfies the SAME `facade-text-content?` as the
   2025-built one.
3. **Field-name collisions forced two renames on `facade-tool`.** The façade tool field for
   `ToolExecution` is named **`exec`** (accessor `facade-tool-exec`) and the annotations field is
   **`annots`** (accessor `facade-tool-annots`) — because `facade-tool-execution` and
   `facade-tool-annotations` are already the CONSTRUCTORs of the sibling façade structs, and a
   same-named struct accessor would collide at module level. (This mirrors 003's own `tool-exec`/
   `tool-annots` naming.)
4. **`facade-request-meta` is the meta-field type for CLIENT request params ONLY (C1/C2).** Built
   for `call-tool`/`read-resource`/`get-prompt`/`complete` params. A 2025 client params' flat
   `_meta` is split into the envelope: reserved 2026 fields (`protocol-version`/`client-info`/
   `client-capabilities`/`log-level`) are `absent`, `progress-token` and `related-task` are read out
   of the flat map, and all other flat keys land in the envelope `rest`. `create-message` carries a
   PLAIN shared `meta` (+ 2025-only `task`); both `elicit-*` carry NEITHER `task` NOR `meta` in 2026
   (both 2025-only). The test asserts `create-message`/`elicit` metas are NOT `facade-request-meta?`.
5. **`rest` is shared and never refused (C4); empty `rest` is `(hasheq)` not `absent`.** Normalizers
   copy `rest` straight through; denormalizers emit it. Cross-revision denormalize of a façade
   carrying a non-empty `rest` (named 2026-only fields cleared) succeeds and the keys survive —
   tested. For Group-3 `facade-list-roots-result`, `rest` and result-level `meta` are BOTH 2025-only
   (2026 is the bare `{roots}` interface); `denormalize-to-2026` refuses a present `meta` OR a
   non-empty `rest` and emits EXACTLY `{roots}`; `denormalize-to-2025` supplies an empty `hasheq`
   when `rest` is absent (the 2025 struct requires a hash).
6. **`facade-read-resource-result` also carries `next-cursor` as 2026-only** — the delivered 2026
   `read-resource-result` struct adds `next-cursor` (2025's does not), so it is refused on
   `denormalize-to-2025` alongside `ttl-ms`/`cache-scope`/`result-type`.
7. **Method dispatch is `(dispatch-for method revision)` → `(cons parser normalizer)` | `#f`** over a
   `hash[(cons method revision)]`. Both-revisions methods (`tools/call`, `resources/read`,
   `prompts/get`, `completion/complete`, `sampling/createMessage`, `roots/list`) have TWO distinct
   entries; single-revision methods (`initialize`, `logging/setLevel`, `resources/subscribe` @2025;
   `server/discover` @2026) resolve only for their home revision (`#f` otherwise). The test asserts
   the two `tools/call` pairs are NOT `eq?` and each yields the correct revision-shaped façade.
   (`subscriptions/listen` was deliberately left OUT of the dispatch table: its 2026 params struct
   wraps a `subscription-filter` + envelope, which the S1 façade does not yet model as a single
   params façade; adding a mismatched parser/normalizer pair would be incorrect. The
   `facade-subscription-filter` struct itself IS provided for the superset.)
8. **Scope realized vs §4 full inventory.** All TESTABLE primitives (Groups 0–4 in full, the Group-5
   notification params, the Group-6 2025-only and Group-7 2026-only representatives) ship with the
   full 4-function (or single-direction) seam. 58 façade struct predicates are counted by the Part-6
   inventory assertion. The thin request/notification ENVELOPE wrappers (method+payload around the
   params façades — Group 5's request structs) are represented through the params façades and the
   superset union contracts rather than as separate façade structs; the substance (the params/result
   shapes that actually diverge) is fully modeled. This is a pragmatic S1 cut that satisfies every
   acceptance-criteria testable claim; later layers (S3 protocol engine) can add the thin envelope
   wrappers if they need to route on the full request struct rather than the method string.

### Seeded recommendations (all resolved above)
