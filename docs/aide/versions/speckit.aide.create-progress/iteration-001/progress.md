...
- 📋 `mcp/core/types/spec-2025-11-25.rkt` + `spec-2026-07-28.rkt` — per-revision structs + contracts
- 📋 `mcp/core/types/types.rkt` — public types + N1 normalized-superset façade
- 📋 `mcp/core/types/guards.rkt` — JSON-RPC predicates (no batch guard, J3)
- 📋 `mcp/core/errors.rkt` — `exn:fail:mcp[:protocol|:auth]`; exn↔JSON-RPC **encode + decode**
...
- [ ] Error codes + version constants match TS `constants.ts`/`enums.ts` byte-for-byte
