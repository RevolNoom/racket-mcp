...
> - **Warnings are keyed PER COMPILE, NOT stored in a single provider-level slot.** A single mutable provider slot would be overwritten by a second `provider-compile` and directly contradict the S7 two-handles-from-one-provider test (and break item 012, which compiles many schemas through one provider). The committed shape: `provider-compile` computes the ignored-keyword list for *that* schema and **associates it with the produced handle** — e.g. by returning it alongside the handle, or via a provider-held `(hasheq handle → warnings)` weak map, or a `make-racket-native-provider`-level accessor that **takes the handle** `(provider-warnings-for provider handle)`. The implementer picks one and records it in Decisions; whichever is chosen, **two handles compiled from the same provider carry distinct, non-overwritten warning lists** (pinned by the N1 fixture in Part 6).
...
- [ ] **Deferred-keyword policy = ignore-with-warning, UNIFORM across all five, recorded ONCE at compile (C4).** EACH of `pattern`, `minLength`/`maxLength`, `minimum`/`maximum`, `additionalProperties`, `uniqueItems` is **skipped** (does NOT affect accept/reject) and its name is **recorded in the per-compile ignored-keyword list** the test reads via the chosen port-compatible accessor (see the warnings-mechanism note; NOT a `compiled-validator` field). **Uniformity is asserted:** all five exhibit the SAME policy (none rejects while another warns) — for each, `{type:string, <deferred>:…}` on a value the deferred keyword *would* reject is **accepted** (the supported `type` part still rejects a non-string), and the keyword appears in the recorded list. `(check-not-exn (lambda () (provider-compile P <each-deferred-schema>)))` for all five (none is on the reject path). **Warn-once cardinality is asserted on the RECORDED LIST (N2):** a handle compiled from a deferred-keyword schema, `validate`d 3×, has a recorded ignored-keyword list of **identical length after 3 validates as after 0** (validate NEVER appends). If a stderr line is also emitted it uses `eprintf`/`current-error-port` (NOT `log-warning`) so a string-capture is valid (N3) — but the recorded-list length is the load-bearing oracle. Each deferred keyword is **listed by name in the module docs**.
- [ ] **Per-compile-keyed warnings — two handles from one provider stay distinct (N1).** From ONE provider instance, `h1` compiled from `{type:"string", minLength:3}` and `h2` from `{type:"string", pattern:"x"}`: `h1`'s recorded ignored-keyword list names `minLength` (and NOT `pattern`), `h2`'s names `pattern` (and NOT `minLength`) — proving the warnings are keyed per compile/handle, NOT stored in a single provider-level slot that a second compile would overwrite. (A single-mutable-slot impl would pass every other warnings assertion — each reads right after its own compile — while being broken for item 012, which compiles many schemas through one provider; this fixture is what catches it.)
...
- [ ] **C1×C4 interaction — ignored keywords contribute ZERO errors (S-a).** A deferred keyword does NOT add a phantom error to the collect-all list. `{type:"number", minimum:0}` on `"x"` → **exactly 1** error (the `type` failure; `minimum` is ignored and contributes 0 errors — NOT a `"minimum: skipped"` pseudo-error that would push the count to 2). Pins that "skipped" means truly skipped, not "reported as a skip-error".
- [ ] **Recording is compile-time, independent of validate verdict (S-b).** `{type:"string", format:"ipv4"}` on `42` → **exactly 1** error (`type`), AND the handle's recorded ignored-keyword list STILL names `ipv4` even though the value failed `type` — proving the record reflects the schema (a compile-time property), not whether validation passed.
- [ ] **Malformed deferred-keyword VALUE — chosen branch is tested (S-c).** A deferred keyword whose value is itself malformed, e.g. `{type:"string", minLength:"three"}` (non-integer `minLength`): the implementer picks raise-at-compile OR ignore-with-warning, **and the chosen branch has a test** — `check-exn` if raise, accept+record if ignore. The path is NOT left uncovered. (Record the choice in Decisions.)
- [ ] **Malformed NESTED sub-schema — recursion policy pinned (S-d).** `{type:"object", properties:{name:{type:"stringg"}}}` (a bad `type` in a *sub*-schema): pin whether `check-schema-shape` **recurses into sub-schemas** (recommended: recurse → fail-fast at compile → `check-exn` that it raises at compile) vs defers the malformed sub-schema until a value descends into `name`. Recommended = recurse + compile-time raise; whichever is chosen is documented + tested.
- [ ] **`enum` nested in structure carries a located path (S-e).** `{type:"object", properties:{color:{enum:["red","green"]}}}` on `{color:"blue"}` → `validation-errors` with a `validation-error` whose `path` is `'("color")` (enum errors are LOCATED like type errors via the `properties` descent, NOT hard-coded to root `'()`).
- [ ] **Empty `properties` hash vs absent `properties` key (S-f).** `{type:"object", properties:{}}` (present-but-empty `properties`) on `(hasheq 'anything 1)` → **accepts**, no crash (distinguishes an absent `properties` key from an empty `properties` hash; a `hash-ref schema 'properties` WITHOUT a default would crash on the absent-key case — both branches MUST be guarded with a default).
...
3. **Write `mcp/core/validators/from-json-schema.rkt`.** Use `#lang racket/base`. In order:
   ...
   - The provider **struct** implementing `gen:json-schema-validator-provider`. Because item 010's `compiled-validator` is a frozen single-field struct that may NOT be widened, the **per-compile ignored-keyword list lives on item 011's own `racket-native-provider` keyed by the produced handle** (e.g. a provider-held `(hasheq handle → warnings)`, or `provider-compile` returns `(values handle warnings)`, or an accessor `(provider-warnings-for provider handle)`) — **NOT a single provider-level mutable slot** (which a second compile would overwrite, contradicting S7/N1). `provider-compile` runs `check-schema-shape` once (raising on malformed, recording the ignore-list for this handle), then returns a `compiled-validator` whose closure runs `(evaluate schema v '())` and maps empty→`(validation-ok v)` / non-empty→`(validation-errors errs)`. The closure NEVER touches the ignore-list (warn-once = compile-time; validate does not append — N2). If a stderr line is emitted it uses `eprintf` (NOT `log-warning`, so a `current-error-port` capture is valid — N3). Record the chosen keying mechanism in Decisions.
   - A constructor (`make-racket-native-provider`) and the explicit `(provide …)` block (struct predicate/constructor + constructor proc + the per-handle ignored-keyword accessor; NOT the internal `evaluate`/`check-schema-shape` helpers; NOT a re-export of the port — callers `require` the port directly for the result API).
...
;;   on a handle whose warnings you separately inspect via warnings-of).
;; (warnings-of h) -> the per-handle ignored-keyword list/set via the chosen port-compatible accessor
;;   (provider-warnings-for, a second return value, etc. — the exact name is the implementer's per Decisions).
...
The committed policy is **ignore-with-warning, recorded per-compile (keyed to the produced handle), recorded list is the load-bearing warn-once oracle**. The test reads warnings via the chosen port-compatible accessor — call it `(warnings-of h)` in the fixtures below (the actual name/shape is the implementer's per Decisions: a `provider-warnings-for`, a second return value, etc.); it returns the list/set of ignored keywords for that handle. It does NOT read a `compiled-validator` field (item 010's struct is frozen). A `current-error-port` string-capture is a permitted **supplementary** assertion ONLY if the impl emits via `eprintf` (N3) — if the impl uses `log-warning`, the supplementary capture MUST use `make-log-receiver`, not `current-error-port` (which would capture nothing and pass vacuously).
...
- **Uniform across all five deferred keywords.** For EACH of `{type:"string", pattern:"^[A-Z]{3}$"}` on `"ab"`, `{type:"string", minLength:3}` on `"ab"`, `{type:"string", maxLength:1}` on `"abc"`, `{type:"number", minimum:0}` on `-1`, `{type:"number", maximum:100}` on `101`, `{type:"object", properties:{name:{type:"string"}}, additionalProperties:false}` on `{name:"x", extra:"y"}`, `{type:"array", items:{type:"number"}, uniqueItems:true}` on `[1,1]`:
  ...
  - the compile **records** the ignored keyword — assert `(warnings-of h)` for that handle names the deferred keyword.
...
- **N1 per-compile-keyed, two handles from ONE provider stay distinct.** `(define h1 (provider-compile P (hasheq 'type "string" 'minLength 3)))` then `(define h2 (provider-compile P (hasheq 'type "string" 'pattern "x")))` from the SAME `P`. Assert `(member "minLength" (warnings-of h1))` AND NOT `(member "pattern" (warnings-of h1))`; `(member "pattern" (warnings-of h2))` AND NOT `(member "minLength" (warnings-of h2))`. This is the fixture a single-mutable-slot impl FAILS (the second compile would have overwritten the first) while passing every read-right-after-own-compile assertion.
- **N2 warn-once on the RECORDED LIST (conjunction, not either/or).** Compile `h (hasheq 'type "string" 'minLength 3)`. Capture `(define n0 (length (warnings-of h)))`. `validate` h 3× (`(validate h "ab")` etc.). Assert `(= (length (warnings-of h)) n0)` — the recorded list is **identical length after 3 validates as after 0** (validate NEVER appends). This is the primary, load-bearing warn-once check. ADDITIONALLY (supplementary), if a stderr line is emitted via `eprintf`, capture compile-time stderr in an `(open-output-string)` and assert exactly one line, and capture the three validates' stderr and assert it is empty — but the recorded-list length is the oracle.
...
- **S-b recording is compile-time, independent of verdict.** `(define h (provider-compile P (hasheq 'type "string" 'format "ipv4")))`; `(errs-from h 42)` → exactly 1 (`type`), AND `(member "ipv4" (warnings-of h))` is true — the unknown format is recorded even though the value failed `type`.
- **S3 unknown (non-deferred) keyword:** `{type:"string", multipleOf:2}` (or `$ref`, `propertyNames`, an `allOf` combinator) on `"hi"` → ACCEPTS on the supported `type`, routes `multipleOf` through the SAME ignore-with-warning catch-all (skipped + recorded). Assert `(warnings-of h)` names `multipleOf`. `(check-not-exn (lambda () (provider-compile P <unknown-kw-schema>)))`.
- **S2 annotations must NOT warn or suppress:** `(define h (provider-compile P (hasheq 'type "string" 'title "X" 'description "Y" 'default "z")))`; `(validation-errors? (validate h 42))` → `#t` (annotations did NOT suppress the `type` failure), AND `(null? (warnings-of h))` → `#t` (pure annotations are ignored WITHOUT a warning — distinguishes them from deferred/unknown keywords).
...
**S-c malformed deferred-keyword VALUE — the chosen branch IS tested (do not leave uncovered).** A deferred keyword whose value is itself malformed (e.g. `(hasheq 'type "string" 'minLength "three")` — `minLength` a string, not an integer): the implementer picks ONE branch and the test pins it — either treat-as-malformed → `(check-exn exn:fail? (lambda () (provider-compile P …)))`, or treat-as-ignore → `(check-not-exn …)` + accept + the keyword still appears in `(warnings-of h)`. Record the choice in Decisions; the path MUST have a test either way.
...
- [ ] **Deferred-keyword policy verified (C4):** all five (`pattern`/`minLength`/`maxLength`/`minimum`/`maximum`/`additionalProperties`/`uniqueItems`) ignore-with-warning UNIFORMLY (none raises), each recorded in `(warnings-of h)`; warn-ONCE on the RECORDED LIST (N2: list length identical after 3 validates as after 0); each listed in module docs.
- [ ] **Per-compile-keyed warnings verified (N1):** h1={type:string,minLength:3} and h2={type:string,pattern:"x"} from one provider — h1 records `minLength` (not `pattern`), h2 records `pattern` (not `minLength`); distinct, not overwritten.
- [ ] **Warnings emission/capture method verified (N3):** stderr (if any) via `eprintf`/`current-error-port` (capturable) — NOT `log-warning` with a vacuous `current-error-port` capture; recorded-list is the load-bearing oracle.
- [ ] **S-a ignored keyword contributes 0 errors:** `{type:number,minimum:0}` on `"x"` → exactly 1 error (no phantom minimum-skipped error).
- [ ] **S-b recording is compile-time:** `{type:string,format:ipv4}` on `42` → exactly 1 error (type) AND `ipv4` still in `(warnings-of h)`.
- [ ] **Unknown-keyword + annotations verified (S3/S2):** `multipleOf`/`$ref` routed through ignore-with-warning (recorded); annotations (`title`/`description`/`default`) ignored WITHOUT warning (`(warnings-of h)` empty) and do NOT suppress a `type` failure.
- [ ] **S-c malformed deferred-keyword value:** `{type:string,minLength:"three"}` — chosen branch (raise OR ignore+record) is tested.
- [ ] **S-d malformed nested sub-schema:** `{type:object,properties:{name:{type:"stringg"}}}` — recursion policy pinned (recommended: raises at compile).
- [ ] **S-e nested enum located path:** `{type:object,properties:{color:{enum:[red,green]}}}` on `{color:"blue"}` → error path `'("color")`.
- [ ] **S-f empty vs absent properties:** `{type:object,properties:{}}` on `(hasheq 'anything 1)` accepts, no crash (default-guarded `hash-ref`).
...
- [ ] deferred-keyword policy verified (C4): all five ignore-with-warning uniformly (none raises), recorded in (warnings-of h); warn-once on recorded list (3 validates → list length unchanged); listed in module docs
- [ ] per-compile-keyed warnings verified (N1): h1=minLength / h2=pattern from one provider stay distinct (not overwritten)
- [ ] warnings emission/capture verified (N3): eprintf/current-error-port (not vacuous log-warning capture); recorded-list is the oracle
- [ ] S-a ignored kw 0 errors: {type:number,minimum:0} on "x" → exactly 1 error
- [ ] S-b compile-time recording: {type:string,format:ipv4} on 42 → 1 error (type) AND ipv4 still recorded
- [ ] S-c malformed deferred value: {type:string,minLength:"three"} → chosen branch tested (raise or ignore+record)
- [ ] S-d malformed nested sub-schema: {type:object,properties:{name:{type:"stringg"}}} → recursion policy pinned (recommended raises at compile)
- [ ] S-e nested enum located path: {type:object,properties:{color:{enum:[red,green]}}} on {color:"blue"} → path '("color")
- [ ] S-f empty vs absent properties: {type:object,properties:{}} on (hasheq 'anything 1) accepts, no crash
- [ ] unknown-keyword + annotations verified (S3/S2): multipleOf/$ref ignore-with-warning recorded; annotations ignored no-warning ((warnings-of h) empty) + don't suppress type failure
