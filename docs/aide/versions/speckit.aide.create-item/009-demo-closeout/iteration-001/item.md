...
`[exn->jsonrpc-error-jsexpr (-> exn? hash?)]`). So the demo's malformed arm has two valid shapes;
**the spec prescribes shape (A)** and notes (B) as an acceptable alternative the implementer may
choose if (A)'s raised-exn shape proves awkward, recording the choice in Decisions:

- **(A) — decode a genuinely malformed message, catch the raised exn, encode it.** Feed a
  deliberately malformed JSON-RPC message (e.g. a `tools/call`-shaped object missing its required
  `name`, or an `initialize` params with `protocolVersion` of the wrong type) to the appropriate
  `r25:json->…` decoder inside `(with-handlers ([exn:fail? …]) …)`; the decoder's flat contract /
  `h-req` raises (item 003's contracts reject malformed inputs — `queue-001.md:30` "a
  contract-rejection test confirming malformed inputs are rejected"); pass the caught `exn` to
  **`exn->jsonrpc-error-jsexpr`** (`errors.rkt:180–181`) to produce the spec-correct JSON-RPC error
  **object** (a `hasheq` with `code`/`message`, and `data` when present). A contract-violation exn
  is a plain `exn:fail` (not an `exn:fail:mcp`), so `exn->jsonrpc-error` maps it to code
  **`-32603` INTERNAL-ERROR** (verified: `errors.rkt:176` — the non-mcp-error branch emits
  `INTERNAL-ERROR`). The demo prints that error object and the `module+ test` asserts its `code` is
  `-32603` and its `message` is a non-empty string. (`INTERNAL-ERROR = -32603` is reachable through
  the barrel from item 001 — verified `008-…md:644`.)
- **(B) — construct a typed MCP error directly and encode it** (`make-protocol-error` /
  `make-mcp-error`, `errors.rkt:88–94`) to show a *specific* code such as `-32600`
  `INVALID-REQUEST` or `-32602` `INVALID-PARAMS` on the wire, then `exn->jsonrpc-error-jsexpr` it.
  This shows a more spec-meaningful code than the catch-all `-32603`, at the cost of not
  literally driving the malformed message THROUGH the decoder. If the implementer wants BOTH the
  "decoder rejects garbage" story AND a meaningful code, do (A) to prove rejection then map the
  caught exn to a `make-protocol-error` with the appropriate code before encoding — record the
  exact choice and the resulting code in Decisions.

Whichever shape is chosen, the demo MUST end the malformed arm by printing a valid JSON-RPC error
**object** (a jsexpr/`hasheq`, suitable for `write-json`) and the `module+ test` MUST assert its
shape (an integer `code`, a string `message`) — proving the "converted to a correct JSON-RPC error
object" claim is real, not narration.
...
| **3 — malformed → JSON-RPC error** | a deliberately malformed message (constructed inline — e.g. `tools/call` params missing required `name`) OR `error-response.json` consumed as the *shape* reference | `r25:json->call-tool-request` inside `with-handlers` (shape A); `exn->jsonrpc-error-jsexpr` (encode); optionally `make-protocol-error` (shape B) | the malformed input; the caught condition / chosen code; the resulting JSON-RPC error **object** | the produced object is a `hash?` with an integer `code` (e.g. `-32603` for shape A, or the chosen `make-protocol-error` code for shape B) and a non-empty string `message` |
...
- [ ] **Arm 3 — a malformed message is converted to a spec-correct JSON-RPC error object:** the
      demo produces, from a deliberately malformed input, a JSON-RPC error **object** that is a
      `hash?` with an integer `code` and a non-empty string `message`, via the `errors.rkt` ENCODE
      path (`exn->jsonrpc-error-jsexpr`, optionally through `make-protocol-error`). The `module+
      test` asserts `(hash? err-obj)`, `(exact-integer? (hash-ref err-obj 'code))`, and
      `(string? (hash-ref err-obj 'message))` with `(> (string-length …) 0)`. For shape (A)
      (decode-and-catch), additionally assert the code is `-32603` (`INTERNAL-ERROR`, the non-mcp
      contract-violation mapping — `errors.rkt:176`); for shape (B), assert the chosen
      `make-protocol-error` code. The exact shape used is recorded in Decisions.
- [ ] **The malformed arm actually exercises a REJECTION, not a fabricated error:** if shape (A),
      the malformed input must genuinely cause the `r25:json->…` decoder to raise (caught by
      `with-handlers`), i.e. the demo proves item 003's flat contracts reject malformed input
      (`queue-001.md:30`), not merely that `exn->jsonrpc-error` can encode an exn the demo
      hand-rolled. (If shape (B) is chosen alone, note in Decisions that the rejection is
      demonstrated by a separate inline `with-handlers` check rather than by the encoded error's
      provenance.)
...
;; canonical jsexpr equality — unordered object keys, ordered lists
...
;; ---- Arm 3: malformed -> JSON-RPC error object (shape A: decode-and-catch) ----
(define malformed (hasheq 'method "tools/call" 'params (hasheq))) ; missing required name
(define err-obj
  (with-handlers ([exn:fail? exn->jsonrpc-error-jsexpr])
    (r25:json->call-tool-request malformed)
    (error "decoder unexpectedly accepted malformed input"))) ; should not reach
(printf "malformed input: ~a\n" (jsexpr->string malformed))
...
(module+ test
  ...
  ;; arm 3 — the malformed message produced a correct JSON-RPC error object
  ...
  (check-equal? (hash-ref err-obj 'code) INTERNAL-ERROR
                "contract-violation maps to -32603 (shape A)")
  (check-true (and (string? (hash-ref err-obj 'message))
                   (> (string-length (hash-ref err-obj 'message)) 0))
              "error object has non-empty message")
  ;; arm 4
  (check-true (jsexpr=? er-orig er-rt) "error-response envelope round-trips (canonical)"))
```

> The code above is a **specification sketch**, not guaranteed-verbatim-correct source — the
> implementer must run it, confirm the `r25:`-prefixed names resolve through the barrel (step 2),
> confirm `malformed` actually triggers a rejection (if `(hasheq 'method "tools/call" 'params
> (hasheq))` is somehow accepted because `name`'s contract permits absence, pick a more
> clearly-malformed input — e.g. `name` of the wrong type — until the decoder genuinely raises),
> and adjust to match the delivered identifier names. The load-bearing contracts the implemented
> file MUST satisfy are the Acceptance criteria, not this sketch's exact bytes.
...
- **The malformed arm must genuinely raise inside `with-handlers`** — if the chosen malformed input
  is silently accepted, the `(error "decoder unexpectedly accepted …")` line fires and the demo/
  test fails loudly (good — that means the input wasn't actually malformed; pick a worse one). The
  demo must not fabricate the error object from a valid input and call it "malformed → error."
...
- [ ] **Malformed arm genuinely rejects:** confirm the chosen malformed input actually causes the
      `r25:json->…` decoder to raise inside `with-handlers` (the `(error "decoder unexpectedly
      accepted …")` guard line is NOT reached); if it is reached, the input wasn't malformed —
      pick a worse one. Record the malformed input + the resulting error code in Validation Results.
- [ ] **Full-suite regression:** `raco test mcp/core/types/ mcp/core/test/ mcp/core/demo/` → exit
      0, 908 inherited + the demo's new checks, 0 regressions.
...
- `raco test mcp/core/types/ mcp/core/test/ mcp/core/demo/` → 908 inherited + the demo's new
  checks, all passing, exit 0.
...
- [ ] Malformed arm rejects: input = <the malformed jsexpr used>; decoder raised; error code = <-32603 or chosen>
- [ ] Full regression: `raco test mcp/core/types/ mcp/core/test/ mcp/core/demo/` → exit 0, <total> passed (908 inherited + <new>)
...
$ raco test mcp/core/types/ mcp/core/test/                 # baseline regression
  → 908 tests passed, exit 0   (inherited from items 001–008)
...
$ raco test mcp/core/demo/s1-demo.rkt                      # the demo's module+ test
  → <N> tests passed, exit 0

$ raco test mcp/core/types/ mcp/core/test/ mcp/core/demo/  # whole tree incl. demo
  → <total> tests passed, exit 0   (908 inherited, 0 regressed; +<new> from the demo)
```
...
(At minimum, record on delivery: (1) the malformed-arm shape used — (A) decode-and-catch → -32603,
(B) `make-protocol-error` with a chosen code, or a hybrid — and the exact malformed input + the
resulting code; (2) whether arm 4 (error-response envelope round-trip) was included; (3) the final
`raco test` command that covers the demo's location; (4) the demo's final location if it differed
from `mcp/core/demo/s1-demo.rkt`; (5) confirmation that the `r25:`-prefixed decoder names resolved
through the item-008 barrel as expected, or the actual names used if item 008 routed them
differently; (6) the exact replacement text used for the `progress.md` "Parity matrix progression"
section; (7) confirmation that NO new table was added to `roadmap.md` and that G1 was NOT
over-claimed.)
