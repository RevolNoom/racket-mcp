...
| `ListRootsResult` (2556) | `list-roots-result` | `roots` (list `root`) |
| `Root` (2572) | `root` | `uri` (string), `name?` (confirm 2572–2600) |

> NOTE: `ListRootsResult` here is NOT declared `extends Result` (line 2556) — confirm whether it
> carries `_meta`. The 2025 `roots-list-changed-notification` is GONE.
...
- **Upstream work items:** **item 001** (`constants.rkt` — `JSONRPC-VERSION`, the error codes
  `UNSUPPORTED-PROTOCOL-VERSION`/`MISSING-REQUIRED-CLIENT-CAPABILITY`, **and the five new
  `*-META-KEY` constants this item ADDS to it**) and **item 002** (`guards.rkt` — the
