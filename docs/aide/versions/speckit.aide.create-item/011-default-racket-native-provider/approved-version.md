...
- [ ] **Deferred-keyword policy verified (C4):** all five (`pattern`/`minLength`/`maxLength`/`minimum`/`maximum`/`additionalProperties`/`uniqueItems`) ignore-with-warning UNIFORMLY (none raises), each recorded as a SYMBOL in `(provider-warnings-for P h)`; warn-ONCE on the RECORDED LIST (N2: list length identical after 3 validates as after 0); each listed in module docs.
- [ ] **Per-compile-keyed warnings verified (N1):** h1={type:string,minLength:3} and h2={type:string,pattern:"x"} from one provider — `(provider-warnings-for P h1)` has `'minLength` not `'pattern`, h2 the reverse; distinct, not overwritten (weak handle→warnings map).
...
- [ ] **S-b recording is compile-time:** `{type:string,format:ipv4}` on `42` → exactly 1 error (type) AND `'ipv4` still in `(provider-warnings-for P h)`.
- [ ] **Warnings element type = SYMBOLS (S-g):** every `provider-warnings-for` membership check uses symbols (`(memq 'minLength …)`), consistent with schema keys; accessor is `(provider-warnings-for provider handle)` over a weak handle→warnings map.
- [ ] **Unknown-keyword + annotations verified (S3/S2):** `multipleOf`/`$ref` routed through ignore-with-warning (recorded as symbol); annotations (`title`/`description`/`default`) ignored WITHOUT warning (`(provider-warnings-for P h)` empty) and do NOT suppress a `type` failure.
...
- [ ] deferred-keyword policy verified (C4): all five ignore-with-warning uniformly (none raises), recorded as symbols in (provider-warnings-for P h); warn-once on recorded list (3 validates → list length unchanged); listed in module docs
- [ ] per-compile-keyed warnings verified (N1): weak handle→warnings map; h1=minLength / h2=pattern from one provider stay distinct (not overwritten)
- [ ] warnings element type = symbols (S-g): provider-warnings-for returns (listof symbol?); membership via (memq 'minLength …)
...
- [ ] unknown-keyword + annotations verified (S3/S2): multipleOf/$ref ignore-with-warning recorded as symbol; annotations ignored no-warning ((provider-warnings-for P h) empty) + don't suppress type failure
