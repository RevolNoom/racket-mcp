...
- [ ] Passthrough verified (RESULTS): _meta + unknown key PRESERVED on a result round-trip
- [ ] Drop verified (PARAMS): unknown non-_meta key DROPPED on a params round-trip; _meta survives
...
- [ ] Three-way strictness verified: envelope-extra rejected; result-extra preserved; params-extra dropped
- [ ] Per-arm union fixtures verified: 23 arm fixtures (5 ContentBlock, 5 SamplingMessageContentBlock incl. tool_use/tool_result, 4 PrimitiveSchema, 5 EnumSchema, 2 Elicit form|url, 2 ResourceContents) each dispatch to the correct struct
- [ ] Populated nested-capability fixture verified: deep tasks.requests.* values round-trip
- [ ] Field-mapping unit test verified (fixture-independent): serverInfo/inputSchema/uriTemplate/_meta/$schema keys exact; fixture keys copied from TS source (lines cited)
