...
_Seed questions for the implementer to resolve and record here:_
- Final spelling of the public binding names (the screaming-kebab choice above is the
  recommendation; record if changed).
- How the TS-checkout path is resolved in the test (`define-runtime-path` + relative
  walk vs `collection-path`) — record the chosen mechanism and why it is cwd-robust.
- Whether the regex extractor lives inline in the test or in a tiny test helper module
  (it is only used here, so inline is acceptable; record the call).
- Confirmation that no `_meta` key constants were added (scope guard) and the agreed
  home for them (S2 `metadata-utils`).
