...
> **Status:** Living architecture document. Iteration 001.
...
- **Purpose/scope.** All on-wire data shapes: JSON-RPC 2.0 envelopes (request/response/notification/batch/error), MCP primitive types (tool, resource, prompt, sampling, elicitation, roots, completion, logging, progress, cancellation), error codes, both versioned spec type sets, type guards/predicates. Mirrors `core/src/types/*`.
- **Interfaces.**
  ...
  - *Internal — Versioned-spec modules.* `spec-2025-11-25`, `spec-2026-07-28` provide per-revision shapes; a normalization seam exposes a version-agnostic façade to the engine.
...
- **Communication.** All synchronous, pure or near-pure; consumed by reference.
...
- **Interfaces.**
  ...
  - *Internal — HTTP request/SSE handling.* Server role: parse POST bodies, manage SSE event streams, session lifecycle, `Host`/`Origin` validation (DNS-rebinding protection, Security NFR), bearer-token extraction → M14. Client role: POST via HTTP client, consume SSE, resume on reconnect.
...
- **Communication.** Two endpoints relay messages directly via channels — no serialization required, but mimics the async event timing of real transports.
...
#### M12. High-level server (`mcp/server/mcp.rkt`) + low-level server (`mcp/server/server.rkt`) + completable (`mcp/server/completable.rkt`)
- **Purpose/scope.**
  - *Low-level `Server`* (`server.rkt`): direct request-handler registration over the engine. Mirrors `server/server.ts`.
  - *High-level `McpServer`* (`mcp.rkt`): ergonomic `register-tool` / `register-resource` (static URI + URI-template) / `register-prompt`, each returning a handle supporting enable/disable/update/remove; list-changed notifications (`tool`/`resource`/`prompt`); `send-logging-message`. Mirrors `server/mcp.ts`.
  - *Completable* (`completable.rkt`): completions for prompt/resource-template arguments. Mirrors `server/completable.ts`.
- **Interfaces.**
  - *External — Registration interface.* register-tool/resource/prompt (keyword-driven, contract-guarded; G4), returning live handles.
  - *External — Notification interface.* list-changed + logging emitters.
  - *Internal — Engine-binding.* Translate registrations into engine request/notification handlers; wire completions; use M4 (schema) + M3 (validators) for tool I/O; M5a (URI templates) for templated resources; M5b for tool-name validation.
...
- **Interfaces.**
  - *External — Client API interface.* The verbs above, contract-guarded + keyword-driven.
...
| F8 | **Schema advertisement + validation** | register-tool (M12) → schema normalize (M4) → emits wire JSON Schema (advertised in `tools/list`) + validation handle (M3 provider) applied on inbound `tools/call` | sync; dependency-inversion via provider port |
...
| M5 Shared | per-module pure helpers | imported by M7,M8,M11,M12,M13,M14 |
...
| M12 Server | engine-binding, completions wiring | registration/notification API to users; down to M11,M3,M4,M5 |
...
- **Composition over inheritance.** Roles (M12/M13) hold/extend the engine (M11) by composition + generics, not subclassing — idiomatic Racket, mirrors TS behavior without JS-isms.
- **Versioned-spec normalization seam.** M1 hides `2025-11-25` vs `2026-07-28` behind a façade so M11/M12/M13 negotiate version once and operate version-agnostically.
...
- Resumption-token storage strategy for resumable SSE (in-memory vs pluggable store) — an M8 internal detail.
